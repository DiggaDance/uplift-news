...
async function initSchema() {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_articles_created_at ON articles(created_at)`;
}
...
export async function GET() {
  try {
    ...
    return Response.json({
      success: true,
      message: `Schema erstellt. ${inserted} Artikel eingefügt, ${skipped} bereits vorhanden.`,
    });
  } catch (err) {
